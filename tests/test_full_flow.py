...
def test_full_backend_flow():
    ...
    assert details_response.json()["organizer"]["organizer_average_rating"] == 0.0
    assert details_response.json()["organizer"]["organizer_total_reviews"] == 0
    ...
    finish_event_response = requests.patch(
        f"{BASE_URL}/events/{event_id}",
        json=finish_event_payload,
        headers=organizer_headers,
    )

    assert finish_event_response.status_code == 200, finish_event_response.text

    review_payload = {
        "rating": 5,
        "comment": "Super wydarzenie",
    }
    ...
    assert review_response.json()["comment"] == "Super wydarzenie"

    average_response = requests.get(
        f"{BASE_URL}/users/{organizer['id']}/reviews/average",
        headers=participant_headers,
    )

    assert average_response.status_code == 200, average_response.text

    average_data = average_response.json()

    assert average_data["user_id"] == organizer["id"]
    ...
    organizer_data = details_after_review_response.json()["organizer"]
